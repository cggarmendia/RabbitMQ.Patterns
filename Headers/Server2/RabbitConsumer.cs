...
namespace Server2
{
    public class RabbitConsumer : IDisposable
    {
        ...
        private const string QueueName = "Headers.Sample8.Queue2";
        ...
        private void OnRecieved(object sender, BasicDeliverEventArgs deliveryArgs)
        {
            ...
            foreach (var headerKey in deliveryArgs.BasicProperties.Headers.Keys)
            {
                ...
                var value = Encoding.Default.GetString((byte[])headerValue);
                Console.WriteLine("Header - Key: {0}, Value: {1}", headerKey, value);
            }
